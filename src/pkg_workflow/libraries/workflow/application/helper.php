...
class WFApplicationHelper {
     ...
     */
    function getTransitionsForDocumentUser($oDocument, $oUser, $includeBlocked = false) {
     ...
     */
    function getWorkflowStateForDocument ($oDocument, $aOptions = array()) {
     ...
     * @internal tested on 2010-03-19
     */
    function getTransitionsFrom($oState, $aOptions = array()) {
    	$type = 'object';
        if (array_key_exists( 'type', $aOptions)) {
        	$type = $aOptions['type'];
        }
     ...
     */
    function performTransitionOnDocument($oTransition, $oDocument, $oUser,  $context='', $comment='')
    {
        ...
        //CpWorkflowUtil::performSystemActionOnDocument( $oDocument );
        return true;
     ...
     */
    static function performSystemActionOnDocument( $oDocument ) {
        /* Check if document is waiting for system action */
        ...
        if (!empty($selected_transition)) {
            return self::performTransitionOnDocument($transition, $oDocument, null, '');
        }
        
        return true;    
    }  
    
    static function getStateName($stateId) 
    {
    ...
    static function getTransitionLogs($context, $item_id)
    {
    ...
    {
    	return 'Workflow 1.1.3 [library 1.1.3, component 1.1.3]';
    }
